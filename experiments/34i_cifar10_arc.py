"""
Experiment 34i: the exp-34e/g/h calibrated-contrastive arc repeated on
CIFAR-10 (holdout 4 "deer", 16+16 concat spaces, seeds matching the exp-33
cifar10 16p16 run so its reference curves overlay exactly).

Part A (probe suite, 9 spaces):
  supcon+simclr        [supcon16 ; simclr16]        (baseline)
  supcon+hybrid[lam1]  [supcon16 ; simclr+sigreg16(lam1)]
  supcon+hybrid[lam5]  [supcon16 ; simclr+sigreg16(lam5)]
  ss[lam1]+hybrid      [supcon+sigreg16(lam1) ; hybrid5]
  ss[lam5]+hybrid      [supcon+sigreg16(lam5) ; hybrid5]
  hybrid->supres       [supres16(from hybrid1 trunk) ; hybrid1]
  supcon+res-simclr    [supcon16 ; simclr-on-supcon-residual16]
  cls->resfeat         [ss5 ; simclr+sigreg on ss5 residual]
  feat->rescls         [supcon+sigreg on hybrid5 residual ; hybrid5]

Part B (four-statistic power scan, pre + post-discovery) on the five arms
scanned on CIFAR-100: supcon+hybrid[lam5], ss[lam5]+hybrid, hybrid->supres,
cls->resfeat, feat->rescls.

    python experiments/34i_cifar10_arc.py
    python experiments/34i_cifar10_arc.py --quick --fractions 0.01,0.1
"""
import os, sys
...
def main():
    ...
    print(f"exp34i [{ds}] calibrated-contrastive arc, "
    ...
    print("\n----- res-simclr16 (SimCLR on supcon residual) -----")
    ...
    print("\n----- resfeat16 (simclr+sigreg on ss5 residual, lam=5) -----")
    torch.manual_seed(args.seed + 17); np.random.seed(args.seed + 17)
    ...
    print("\n----- rescls16 (supcon+sigreg on hybrid5 residual, lam=5) -----")
    ...
    # arm -> (cls net, cls anchors (len(seen) x D), feat net, feat cents)
    ARMS = {
    ...
    # ----- Part B: power scan on the five arms ------------------------------
    ...
    for stat in STATS:
        print(f"\n===== EXP34I {stat.upper()} POWER SUMMARY "
              f"(alpha={args.alpha}) =====")
        ...
        if ref is not None:
            rf = ref["fractions"]
            key = f"{stat}_supcon+simclr_post"
            if key in ref:
                plt.plot(rf, ref[key], ":", color="gray", lw=1.3,
                         label="supcon+simclr post (exp33)")
        plt.xscale("log")
        plt.axhline(0.05, color="gray", lw=1, ls=":")
        plt.xlabel("injected anomaly fraction")
        plt.ylabel("power at alpha=0.05")
        plt.title(f"exp34i [cifar10] calibrated-contrastive arc: {stat} power")
        plt.grid(alpha=0.25, which="both")
        plt.legend(loc="upper left", fontsize=7, ncol=2)
        plt.tight_layout()
        out = plot_path(f"exp34i_{stat}_power_cifar10.png")
    ...
    np.savez(os.path.join(outdir, "arc_cifar10.npz"), **npz)
    print(f"saved {outdir}/arc_cifar10.npz")
