"""
Repulsive floating means for CIFAR/ResNet 32-dim SIGReg, seeded 3 sigma apart.

Class means are initialized on orthogonal anchors with pairwise distance
`--pair-dist` (default 3.0 = 3 sigma of the unit within-class Gaussian target,
i.e. anchor norm pair_dist/sqrt(2)) and are learnable.  Two mean-geometry
variants are compared at identical seeding:

    sigreg-repulse : inverse-square (Coulomb-like) repulsion between means
                     + mild shrinkage toward the origin (mode="repulse")
    sigreg-free    : no geometry term at all (mode="free") -- does a tight
                     3-sigma seed collapse without repulsion?

Same protocol as runs 09/10: CIFAR-pretrained ResNet backbone, inclusive
(10-way micro-AUC) and hold-out-deer (binary vs-rest) evaluations.

Reference numbers (10 ssl epochs, 5 probe epochs, seed 0):
    SIGReg + hinge, 5-scale seed (09)   inclusive 0.9645   holdout 0.8407
    SIGReg no hinge, 5-scale seed (10)  inclusive 0.9655   holdout 0.8324
    SupCon + aug (09)                   inclusive 0.9920   holdout 0.9290
    SupCon no aug (10)                  inclusive 0.9904   holdout 0.9143

Outputs (plots/):
    roc_cifar_repulse_inclusive.png / roc_cifar_repulse_holdout.png
    corner_cifar_repulse_<method>_<case>.png (first 10 latent dims)
"""
import os, sys
...
MODES = {"sigreg-repulse": "repulse", "sigreg-free3": "free"}
...
def make_backbone(loader, method, ssl_ep, args):
    scale = args.pair_dist / math.sqrt(2.0)      # orthogonal anchors: pairwise = scale*sqrt(2)
    ...
    train_sigreg_classwise(backbone, loader, ssl_ep, means,
                           learn_means=True, mode=MODES[method])
    return backbone


def inclusive(method, ssl_ep, probe_ep, args):
    print(f"\n=== CIFAR repulse-study inclusive: {method} ===")
    ...
    plot_corner(embs[:, :CORNER_DIMS], elab, plot_path(f"corner_cifar_repulse_{method}_inclusive.png"),
...
def holdout(method, ssl_ep, probe_ep, args):
    print(f"\n=== CIFAR repulse-study holdout ({CIFAR_CLASSES[HOLDOUT]}): {method} ===")
    ...
    plot_corner(embs[:, :CORNER_DIMS], ish, plot_path(f"corner_cifar_repulse_{method}_holdout.png"),
...
def main():
    ...
    ap.add_argument("--pair-dist", type=float, default=3.0,
                    help="pairwise distance between seed means, in units of the "
                         "unit within-class sigma")
    args = ap.parse_args()
    ...
    print(f"device={DEVICE}  arch={args.arch}  pretrain={args.pretrain}  "
          f"emb_dim={EMB_DIM}  seed pair-dist={args.pair_dist} sigma")

    methods = list(MODES)
    inc = {m: inclusive(m, ssl_ep, probe_ep, args) for m in methods}
    hold = {m: holdout(m, ssl_ep, probe_ep, args) for m in methods}

    overlay(inc, "CIFAR-10 inclusive: repulsive vs free means, 3-sigma seed",
            "roc_cifar_repulse_inclusive.png")
    overlay(hold, f"CIFAR-10 hold-out '{CIFAR_CLASSES[HOLDOUT]}' vs rest, 3-sigma seed",
            "roc_cifar_repulse_holdout.png")

    print("\n===== REPULSE SUMMARY (vs runs 09/10) =====")
    print("  SIGReg + hinge, seed 5-scale   inclusive micro-AUC=0.9645   holdout AUC=0.8407")
    print("  SIGReg no hinge, seed 5-scale  inclusive micro-AUC=0.9655   holdout AUC=0.8324")
    print("  SupCon + aug                   inclusive micro-AUC=0.9920   holdout AUC=0.9290")
    print("  SupCon no aug                  inclusive micro-AUC=0.9904   holdout AUC=0.9143")
    for m in methods:
