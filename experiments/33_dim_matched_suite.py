...
def main():
    ...
    seen = [c for c in range(10) if c not in holdouts]
    ...
    print(f"exp33 [dim-matched] single={args.dim_single}d "
    ...
    means_joint16 = make_anchors(cfg16["pair_dist"] / math.sqrt(2.0),
                                 emb_dim=args.dim_single, n_classes=10).clone()
    ...
    plot_latent_panels({n: (tests[n], te_lab) for n in arm_names}, holdouts,
                       CIFAR_NAMES, plot_path(f"latent_cifar10_exp33_{tag}.png"),
    ...
    plt.tight_layout()
    plt.savefig(plot_path(f"exp33_novelty_roc_{tag}.png"), dpi=150); plt.close()
    for name in arm_names:
        te = tests[name]
        ...
        plot_corner(sl, te_lab,
                    plot_path(f"corner_cifar10_exp33_{tag}_"
    ...
    for name in arm_names:
        ...
        pe = exp30.power_at_alpha(s[bg_mask], s[tel == args.holdout],
                                  args.alpha)
        ...
        maha_fn, mmd_fn, n_bg, n_sig = exp32.make_stats_fns(
            tr, trl, te, tel, seen, args.holdout, args.seed)
        print(f"  [{name}] maha")
    ...
    for i_f, f in enumerate(fractions):
        ...
        for name in arm_names:
            ...
            if aug is None:
                ...
                d_disc = torch.cdist(zt, cur_means[10:]).min(1).values
            else:
                ...
                disc_anc = torch.cat([cur_means[10:], disc_ssl], dim=1)
            ...
            pe = exp30.power_at_alpha(s[bg_mask],
                                      s[tel_post == args.holdout], args.alpha)
            ...
            bg = torch.as_tensor(te_post[bg_mask], dtype=torch.float32,
                                 device=DEVICE)
            sg = torch.as_tensor(te_post[tel_post == args.holdout],
                                 dtype=torch.float32, device=DEVICE)
            ...
            maha_fn, mmd_fn, n_bg, n_sig = exp32.make_stats_fns(
                tr_post, trl_post, te_post, tel_post, seen, args.holdout,
                args.seed + i_f)
    ...
    for stat in STATS:
        ...
        plt.tight_layout()
        plt.savefig(plot_path(f"exp33_{stat}_power_{tag}.png"), dpi=150)
        plt.close()
        print("  saved " + plot_path(f"exp33_{stat}_power_{tag}.png"))
    ...
    np.savez(os.path.join("logs", "exp33", f"power_data_{tag}.npz"),
