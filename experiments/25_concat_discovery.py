"""
Anchor discovery in the CONCATENATED space (CIFAR-100).

Combines the dual-space architecture (exp 22) with the iterated discovery
loop (exp 24).  Two 32-dim spaces trained on the same input:

    SSL space   : SIGReg-SSL trunk (invariance + global N(0,I); no labels;
                  holdouts excluded).  Stays FROZEN — the stable reference.
    sup space   : a copy of the trunk fine-tuned with classwise SIGReg + proto
                  (w=1, CIFAR-100 recipe).

Concat = [sup ; ssl] (64d).  Anchors in the concat space are
[learned mean_c ; SSL class centroid_c]; discovered anchors come from k-means
(BIC-selected k) on the outlier pool of the concat distances.  Each round:
pool -> cluster -> append anchors -> pseudo-label pooled points by nearest
discovered anchor -> fine-tune the SUP branch with the extended means (sup
halves of the discovered anchors) -> refresh the concat anchors.

Rationale: a novel class disguised inside a seen shell of the supervised space
is still displaced in the SSL space (decorrelated failure modes, exp 22), so
the concat pool should be purer than the sup-only pool — the fix small-k
CIFAR-100 discovery needs.

Outputs:
    plots/concat_discovery_cifar100.png
"""
import os, sys
sys.path.insert(0, os.path.dirname(os.path.dirname(os.path.abspath(__file__))))
import argparse
import copy
import math
import numpy as np
import torch
import torch.nn as nn
import matplotlib
matplotlib.use("Agg")
import matplotlib.pyplot as plt
from sklearn.metrics import roc_auc_score
from torch.utils.data import DataLoader, Dataset

from supersig.config import plot_path, DATA_DIR, DEVICE
from supersig.data import (
    get_cifar_loaders, cifar_balanced_loader, cifar_two_view_loader,
    BalancedBatchSampler, _cifar_spec,
)
from supersig.models import CIFARResNetBackbone
from supersig.losses import make_anchors
from supersig.train import (
    train_sigreg_ssl, train_sigreg_hybrid, collect_embeddings, REP_WEIGHT,
)

DIM = 32
N_CLASSES = 100
PAIR_DIST = 5.0
DATASET = "cifar100"
HOLDOUT_SETS = {1: [4], 3: [4, 30, 70],
                10: [4, 14, 24, 34, 44, 54, 64, 74, 84, 94],
                20: [4 + 5 * i for i in range(20)]}


class Stack(nn.Module):
    def __init__(self, trunk, head):
        super().__init__()
        self.trunk, self.head = trunk, head

    def forward(self, x):
        return self.head(self.trunk(x))


class PseudoDataset(Dataset):
    def __init__(self, base, indices, labels):
        self.base, self.indices, self.labels = base, indices, labels

    def __len__(self):
        return len(self.indices)

    def __getitem__(self, i):
        img, _ = self.base[self.indices[i]]
        return img, int(self.labels[i])


def kmeans(X, k, iters=30, seed=0):
    g = torch.Generator().manual_seed(seed)
    idx = [int(torch.randint(len(X), (1,), generator=g))]
    for _ in range(k - 1):
        d2 = torch.cdist(X, X[idx]).min(1).values ** 2
        idx.append(int(torch.multinomial((d2 / d2.sum()).cpu(), 1, generator=g)))
    C = X[idx].clone()
    for _ in range(iters):
        a = torch.cdist(X, C).argmin(1)
        for j in range(k):
            if (a == j).any():
                C[j] = X[a == j].mean(0)
    return C, torch.cdist(X, C).argmin(1)


def bic_select(X, kmax=4, seed=0):
    n, d = X.shape
    best = None
    for k in range(1, kmax + 1):
        C, a = kmeans(X, k, seed=seed + k)
        ll = -0.5 * ((X - C[a]) ** 2).sum().item() - 0.5 * n * d * math.log(2 * math.pi)
        bic = -2 * ll + k * d * math.log(n)
        if best is None or bic < best[1]:
            best = (k, bic, C, a)
    return best[0], best[2], best[3]


def class_centroids(Z, labels, classes):
    return torch.stack([Z[torch.as_tensor(labels == c, device=DEVICE)].mean(0)
                        for c in classes])


def main():
    ap = argparse.ArgumentParser()
    ap.add_argument("--quick", action="store_true")
    ap.add_argument("--limit", type=int, default=None)
    ap.add_argument("--ssl-epochs", type=int, default=None)
    ap.add_argument("--sup-epochs", type=int, default=None)
    ap.add_argument("--ft-epochs", type=int, default=None)
    ap.add_argument("--rounds", type=int, default=2)
    ap.add_argument("--seed", type=int, default=0)
    ap.add_argument("--arch", default="resnet20")
    ap.add_argument("--pretrain", default="cifar100")
    ap.add_argument("--ks", default="1,3,10,20")
    ap.add_argument("--tau-quantile", type=float, default=0.95)
    ap.add_argument("--kmax", type=int, default=4)
    args = ap.parse_args()
    ssl_ep = args.ssl_epochs or (2 if args.quick else 20)
    sup_ep = args.sup_epochs or (2 if args.quick else 10)
    ft_ep = args.ft_epochs or (1 if args.quick else 5)
    ks = [int(x) for x in args.ks.split(",")]
    rep_w = REP_WEIGHT * 45.0 / (N_CLASSES * (N_CLASSES - 1) / 2)

    from torchvision import datasets as tvd
    names = tvd.CIFAR100(DATA_DIR, train=False, download=True).classes
    train_loader, test_loader = get_cifar_loaders(quick=args.quick, limit=args.limit,
                                                  dataset=DATASET)
    train_eval_loader = DataLoader(train_loader.dataset, batch_size=256,
                                   shuffle=False, num_workers=2)
    cls, plain, _ = _cifar_spec(DATASET)
    base = cls(DATA_DIR, train=True, download=True, transform=plain)

    history = {}
    for k in ks:
        holdouts = set(HOLDOUT_SETS[k])
        seen = [c for c in range(N_CLASSES) if c not in holdouts]
        print(f"\n===== k={k} ({', '.join(names[c] for c in sorted(holdouts))}) =====")
        torch.manual_seed(args.seed); np.random.seed(args.seed)
        trunk = CIFARResNetBackbone(DIM, arch=args.arch,
                                    pretrain=args.pretrain).to(DEVICE)
        train_sigreg_ssl(trunk, cifar_two_view_loader(
            quick=args.quick, labeled=False, holdout=holdouts,
            limit=args.limit, dataset=DATASET), ssl_ep)

        torch.manual_seed(args.seed + 1); np.random.seed(args.seed + 1)
        sup = Stack(copy.deepcopy(trunk),
                    nn.Sequential(nn.Linear(DIM, 64), nn.ReLU(),
                                  nn.Linear(64, DIM)).to(DEVICE))
        means = make_anchors(PAIR_DIST / math.sqrt(2.0), emb_dim=DIM,
                             n_classes=N_CLASSES).clone()
        train_sigreg_hybrid(sup, cifar_balanced_loader(
            DATASET, holdout=holdouts, quick=args.quick, limit=args.limit),
            sup_ep, means, mode="repulse", disc="proto", alpha=1.0,
            rep_weight=rep_w, sigreg_weight=1.0, n_slices=64)

        ssl_tr, tr_lab = collect_embeddings(trunk, train_eval_loader)
        ssl_te, te_lab = collect_embeddings(trunk, test_loader)
        Zssl_tr = torch.as_tensor(ssl_tr, device=DEVICE)
        Zssl_te = torch.as_tensor(ssl_te, device=DEVICE)
        is_seen_lab = np.isin(tr_lab, seen)
        ssl_cents = class_centroids(Zssl_tr[torch.as_tensor(is_seen_lab, device=DEVICE)],
                                    tr_lab[is_seen_lab], seen)

        cur_means = means.detach().clone()      # sup-space means (grows)
        disc_ssl = None                          # ssl halves of discovered anchors
        pooled = np.zeros(len(train_loader.dataset), dtype=bool)
        history[k] = []
        for r in range(1, args.rounds + 1):
            sup_tr, _ = collect_embeddings(sup, train_eval_loader)
            Zsup = torch.as_tensor(sup_tr, device=DEVICE)
            Zcat = torch.cat([Zsup, Zssl_tr], dim=1)
            seen_anchors = torch.cat([cur_means[seen], ssl_cents], dim=1)
            if disc_ssl is not None:
                disc_anchors = torch.cat([cur_means[N_CLASSES:], disc_ssl], dim=1)
                anchor_mat = torch.cat([seen_anchors, disc_anchors])
            else:
                anchor_mat = seen_anchors
            dmin = torch.cdist(Zcat, anchor_mat).min(1).values
            tau = torch.quantile(dmin[torch.as_tensor(is_seen_lab, device=DEVICE)],
                                 args.tau_quantile)
            pool = (dmin > tau).cpu().numpy()
            purity = (~is_seen_lab[pool]).mean() if pool.any() else float("nan")
            kmax = max(args.kmax, k + 2)
            khat, centers, _ = bic_select(Zcat[torch.as_tensor(pool, device=DEVICE)],
                                          kmax=kmax, seed=args.seed + r)
            new_sup, new_ssl = centers[:, :DIM], centers[:, DIM:]
            cur_means = torch.cat([cur_means, new_sup.detach()], dim=0)
            disc_ssl = new_ssl.detach() if disc_ssl is None else torch.cat(
                [disc_ssl, new_ssl.detach()])
            pooled |= pool
            # pseudo-labels: nearest discovered concat anchor for pooled points
            disc_anchors = torch.cat([cur_means[N_CLASSES:], disc_ssl], dim=1)
            p_idx = np.where(pooled)[0]
            p_lab = N_CLASSES + torch.cdist(
                Zcat[torch.as_tensor(pooled, device=DEVICE)],
                disc_anchors).argmin(1).cpu().numpy()
            lab_idx = np.where(is_seen_lab)[0]
            ft_idx = np.concatenate([lab_idx, p_idx])
            ft_lab = np.concatenate([tr_lab[lab_idx], p_lab])
            sampler = BalancedBatchSampler(list(ft_lab), n_classes=25, n_per_class=24)
            ft_loader = DataLoader(PseudoDataset(base, ft_idx, ft_lab),
                                   batch_sampler=sampler, num_workers=2)
            train_sigreg_hybrid(sup, ft_loader, ft_ep, cur_means, mode="repulse",
                                disc="proto", alpha=1.0, rep_weight=rep_w,
                                sigreg_weight=1.0, n_slices=64)
            cur_means = cur_means.detach()
            # refresh discovered ssl halves from current members
            sup_tr, _ = collect_embeddings(sup, train_eval_loader)
            Zsup = torch.as_tensor(sup_tr, device=DEVICE)
            for j in range(disc_ssl.size(0)):
                m = torch.as_tensor(pooled, device=DEVICE) & torch.as_tensor(
                    np.isin(np.arange(len(tr_lab)), p_idx[p_lab == N_CLASSES + j]),
                    device=DEVICE)
                if m.any():
                    disc_ssl[j] = Zssl_tr[m].mean(0)

            # evaluate
            sup_te, _ = collect_embeddings(sup, test_loader)
            Zcat_te = torch.cat([torch.as_tensor(sup_te, device=DEVICE), Zssl_te], dim=1)
            seen_anchors = torch.cat([cur_means[seen], ssl_cents], dim=1)
            disc_anchors = torch.cat([cur_means[N_CLASSES:], disc_ssl], dim=1)
            d_seen = torch.cdist(Zcat_te, seen_anchors).min(1).values
            d_each = torch.cdist(Zcat_te, disc_anchors)
            d_new = d_each.min(1).values
            is_unseen = np.isin(te_lab, list(holdouts)).astype(int)
            after = roc_auc_score(is_unseen, (d_seen - d_new).cpu().numpy())
            per_class = {}
            for c in sorted(holdouts):
                counts = [int(((te_lab == c) &
                               (d_each.argmin(1).cpu().numpy() == j)).sum())
                          for j in range(disc_anchors.size(0))]
                j = int(np.argmax(counts))
                per_class[c] = roc_auc_score((te_lab == c).astype(int),
                                             (-d_each[:, j]).cpu().numpy())
            mean_pc = float(np.mean(list(per_class.values())))
            history[k].append(dict(r=r, pool=int(pool.sum()), purity=purity,
                                   khat=khat, after=after, mean_pc=mean_pc))
            print(f"  round {r}: pool={pool.sum()} purity={purity:.3f} k-hat={khat}"
                  f"  margin AUC={after:.4f}  mean anchor AUC={mean_pc:.4f}")
            if len(per_class) <= 5:
                print("           " + "  ".join(
                    f"{names[c]}={a:.3f}" for c, a in per_class.items()))

    print("\n===== CONCAT-DISCOVERY SUMMARY =====")
    print(f"{'k':>3}{'round':>6}{'pool':>7}{'purity':>8}{'k-hat':>6}"
          f"{'margin':>9}{'mean-pc':>9}")
    for k in ks:
        for h in history[k]:
            print(f"{k:>3}{h['r']:>6}{h['pool']:>7}{h['purity']:>8.3f}"
                  f"{h['khat']:>6}{h['after']:>9.4f}{h['mean_pc']:>9.4f}")

    plt.figure(figsize=(7, 5))
    for i, k in enumerate(ks):
        rs = [h["r"] for h in history[k]]
        plt.plot(rs, [h["after"] for h in history[k]], f"C{i}-o", lw=2,
                 label=f"k={k} margin")
        plt.plot(rs, [h["mean_pc"] for h in history[k]], f"C{i}--^", lw=1.5,
                 alpha=0.8, label=f"k={k} mean anchor")
    plt.xticks(range(1, args.rounds + 1))
    plt.xlabel("discovery round"); plt.ylabel("AUC")
    plt.title("CIFAR-100: discovery in the concatenated space")
    plt.legend(fontsize=8); plt.grid(alpha=0.3); plt.tight_layout()
    plt.savefig(plot_path("concat_discovery_cifar100.png"), dpi=150); plt.close()
    print(f"\n  saved {plot_path('concat_discovery_cifar100.png')}")
    print("Done.")


if __name__ == "__main__":
    main()
