"""
Experiment 34h: the calibrated-contrastive (supcon+sigreg / simclr+sigreg)
style applied to the RESIDUAL construction, in both training orders:

  cls->resfeat : classifier first (supcon+sigreg16), then the featurizer as
                 SimCLR+SIGReg on the residual z - mean_y (frozen classifier
                 centroids).  Contrastive-calibrated analog of sup->res.
  feat->rescls : featurizer first (simclr+sigreg16, lam5), then the
                 classifier as SupCon+SIGReg on the residual z - mean_y
                 (frozen featurizer centroids).  Analog of ssl->supres but
                 through the residual rather than a warm start.

Both evaluated as 16+16 concats [classifier ; featurizer] with the usual
suite: probe/acc/maha/gaussianity (Part A) and the four-statistic power
scan, pre + post-discovery (Part B).  References: supcon+hybrid probe
0.9423, SparKer/MMD 1.00 at f=0.05, MMD post 0.88 at f=0.02 (exps 34e/34f).

    python experiments/34h_residual_orderings.py [--cls-lam 5.0]
    python experiments/34h_residual_orderings.py --quick --fractions 0.01,0.05
"""
import os, sys
...
REF = {"supcon+hybrid[lam5] (34e)": 0.9423, "supcon+simclr (r1)": 0.9394,
       "supcon (r1)": 0.9268}
...
def main():
    ...
    ap.add_argument("--cls-lam", type=float, default=5.0,
                    help="sigreg weight of the supcon+sigreg classifier "
                         "halves (set from the exp-34g Part-A winner)")
    ...
    print(f"exp34h [{ds}] residual orderings, holdout={sorted(holdouts)}, "
          f"cls_lam={args.cls_lam}")
    ...
    def labeled_loader():
        return cifar_two_view_loader(quick=args.quick, labeled=True,
                                     holdout=holdouts, dataset=ds)

    # ----- arm 1: cls -> resfeat -------------------------------------------
    print(f"\n----- ss16 supcon+sigreg (lam={args.cls_lam}) -----")
    torch.manual_seed(args.seed + 15); np.random.seed(args.seed + 15)
    ss = backbone()
    train_supcon_sigreg(ss, labeled_loader(), sup_ep, lam=args.cls_lam,
                        n_slices=cfgH["n_slices"])
    ss_cents = cents_of(ss)
    print("\n----- resfeat16 simclr+sigreg on ss residual (lam=5) -----")
    torch.manual_seed(args.seed + 17); np.random.seed(args.seed + 17)
    resfeat = copy.deepcopy(ss)
    means_ss = exp28.fill_means(ss_cents, seen, cfgH).to(DEVICE)
    train_simclr_residual(resfeat, labeled_loader(), con_ep, means_ss,
    ...
    print(f"\n----- rescls16 supcon+sigreg on hybrid residual "
          f"(lam={args.cls_lam}) -----")
    torch.manual_seed(args.seed + 18); np.random.seed(args.seed + 18)
    rescls = copy.deepcopy(hybrid)
    means_hyb = exp28.fill_means(hybrid_cents, seen, cfgH).to(DEVICE)
    train_supcon_sigreg_residual(rescls, labeled_loader(), sup_ep, means_hyb,
                                 lam=args.cls_lam, n_slices=cfgH["n_slices"])
    rescls_cents = cents_of(rescls)

    # arm -> (cls net, cls cents, feat net, feat cents)
    ARMS = {
        "cls->resfeat": (ss, ss_cents, resfeat, resfeat_cents),
        "feat->rescls": (rescls, rescls_cents, hybrid, hybrid_cents),
    }
    arm_names = list(ARMS)
    ...
    # ----- Part B: power scan on both arms ----------------------------------
    pre_power = {s: {} for s in STATS}
    for name in arm_names:
        c_net, c_cents, f_net, f_cents = ARMS[name]
    ...
    for i_f, f in enumerate(fractions):
        ...
        for name in arm_names:
            ...
            _, extras = exp28.run_concat_discovery(
                bb, f_net, exp28.fill_means(c_cents, seen, cfgH).clone(),
                f_cents, base=sub, dim=args.dim_half,
    ...
    for stat in STATS:
        print(f"\n===== EXP34H {stat.upper()} POWER SUMMARY "
              f"(alpha={args.alpha}) =====")
        ...
        if ref34f is not None:
            rf = ref34f["fractions"]
            key = f"{stat}_supcon+hybrid_post"
            if key in ref34f:
                plt.plot(rf, ref34f[key], ":", color="gray", lw=1.3,
                         label="supcon+hybrid post (34f)")
        plt.xscale("log")
        plt.axhline(0.05, color="gray", lw=1, ls=":")
        plt.xlabel("injected anomaly fraction")
        plt.ylabel("power at alpha=0.05")
        plt.title(f"exp34h [cifar100] residual orderings: {stat} power"
                  " (train-side clamp above f~0.01)")
        plt.grid(alpha=0.25, which="both")
        plt.legend(loc="upper left", fontsize=8)
        plt.tight_layout()
        out = plot_path(f"exp34h_{stat}_power_cifar100.png")
    ...
    np.savez(os.path.join(outdir, "power_data_residual_orderings.npz"), **npz)
    print(f"saved {outdir}/power_data_residual_orderings.npz")
