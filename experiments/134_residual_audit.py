...
def main():
    ...
    if args.selftest:
        _selftest(); return
    if not (args.parent and args.child):
        ap.error("need --parent and --child (or --selftest)")

    dp, dc = (np.load(args.parent, allow_pickle=True),
              np.load(args.child, allow_pickle=True))
    tr_lab, te_lab = np.asarray(dp["tr_lab"]), np.asarray(dp["te_lab"])
    ...
    ratio = half_scale_ratio(dp["tr"], dc["tr"])
    ...
    scan = combine_scan(dp["tr"], dc["tr"], dp["te"], dc["te"],
                        tr_lab, te_lab, seen, hold)
    print(f"\n{'mode':>13s}{'probe':>9s}{'eucl':>9s}{'dim':>6s}")
    for m, r in scan.items():
        print(f"{m:>13s}{r['probe']:>9.4f}{r['eucl']:>9.4f}{r['dim']:>6d}")

    os.makedirs(args.out, exist_ok=True)
    with open(os.path.join(args.out, f"combine{run_tag()}.json"), "w") as fh:
        json.dump(dict(half_scale_ratio=ratio, scan=scan), fh, indent=1,
                  default=float)
    print(f"\nwrote {args.out}/combine{run_tag()}.json")
