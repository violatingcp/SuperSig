"""Tests for the exp-138 base-rate estimator work.
...
_P = os.path.join(os.path.dirname(os.path.dirname(os.path.abspath(__file__))),
                  "experiments", "138_base_rate_estimator.py")
_s = importlib.util.spec_from_file_location("exp138", _P)
exp138 = importlib.util.module_from_spec(_s)
_s.loader.exec_module(exp138)


def _case(b=0.02, overlap=0.0, seed=0):
    z, v = exp138._make(b, sep=8.0, overlap=overlap, seed=seed)
    f = exp138._analytic_f(z, 8.0 * (1 - overlap), b)
    return z, v, f
...
def test_estimators_are_exact_without_overlap(est):
    _, v, f = _case(b=0.02)
    assert exp138.b_hat(f, f[~v], est) == pytest.approx(0.02, abs=0.004)
...
def test_estimators_never_overstate(est):
    ...
    for ov in (0.0, 0.5, 0.8, 0.95):
        ...
        assert exp138.b_hat(f, f[~v], est) <= 0.02 * 1.15, (est, ov)
...
def test_n_min_scan_purity_falls_as_n_min_rises():
    ...
    rows = exp138.n_min_scan(f, ~v, v, n_mins=(10, 30, 100))
...
def test_n_min_scan_reports_implied_alpha():
    _, v, f = _case(b=0.02, overlap=0.55)
    rows = exp138.n_min_scan(f, ~v, v, n_mins=(10, 30))
    for r in rows:
